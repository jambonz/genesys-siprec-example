...
const parseCallData = (logger, prefix, obj) => {
  const ret = {};
  ...
  logger.debug({ callData: ret }, 'parsed callData');
  return ret;
};
 ...
 * @return {Promise}
 */
const parseSiprecPayload = (logger, req) => {
  const {payload} = req.body.sip;
  const opts = {};
  return new Promise((resolve, reject) => {
    ...
    if (!meta && sdp) {
      ...
      opts.sessionId = v4();
      logger.info({ payload }, 'SIPREC payload with no metadata (e.g. Cisco NBR)');
      resolve(opts);
    } else if (!sdp || !meta) {
      logger.info({ payload }, 'invalid SIPREC payload');
    ...
    xmlParser(meta, (err, result) => {
      ...
      try {
        if (typeof result === 'object' && Object.keys(result).length === 1) {
          ...
          const callData = parseCallData(logger, prefix, obj);
          if (callData) {
            logger.debug(`callData: ${JSON.stringify(callData)}`);
            ...
            logger.debug(`opts.caller from callData: ${JSON.stringify(opts.caller)}`);
            logger.debug(`opts.callee from callData: ${JSON.stringify(opts.callee)}`);
          }
      ...
      logger.debug(opts, 'payload parser results');
      resolve(opts) ;
    }) ;
  }) ;
};
